...
const processSuppIntakeData = (workoutData: WorkoutData[]) => {
  ...
  workoutData.forEach((workout) => {
    ...
    if (intakeInfo) {
      ...
      if (dayIndex >= 0) {
        // -1 bedeutet Sonntag, hier nicht berücksichtigt
        datesOfWeek[dayIndex] = date.toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        });
        if (intakeInfo.bcaa)
          supplementTotals["BCAA"][dayIndex] += intakeInfo.bcaaAmount;
        if (intakeInfo.creatin)
          supplementTotals["Creatin"][dayIndex] += intakeInfo.creatinAmount;
        if (intakeInfo.eaa)
          supplementTotals["EAA"][dayIndex] += intakeInfo.eaaAmount;
        if (intakeInfo.protein)
          supplementTotals["Protein"][dayIndex] += intakeInfo.proteinAmount;
      }
    }
  });
  ...
  const labels = daysOfWeek.map(
    (day, index) => `${day} (${datesOfWeek[index]})`,
  );
